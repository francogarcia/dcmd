...
No momento, o projeto está em algo como /open beta/.
A intenção é promover a parte interativa e investigativa do website.
Ou seja, enxugar o texto e melhorar o /layout/.
...
O /script/ =./dados-abertos-da-camara/preparacao-dados.sh= remove o diretório =./dados-abertos-da-camara/output/=.

Este diretório será criado pelo /script/.
Entretanto, se você (hipoteticamente) tiver um diretório com este nome em =./dados-abertos-da-camara/=, por favor, tome as devidas precauções.
...
git clone https://github.com/francogarcia/dcmd
pushd "dcmd"

pushd "dados-abertos-da-camara"
# NOTE Novamente, cuidado com o "rm -r ./output".
./preparacao-dados.sh
#+END_SRC
...
Como não há dados abertos de salários ([[https://dcmd.francogarcia.com/#sobre-os-dados][solicite no Fale Conosco da Câmara]]), o projeto tenta estimar os salários usando a API de históricos para calcular os dias em que cada deputado esteve na situação "Exercício".
Isso é feito pelos /scripts/ =historicos-deputados.py= (obtém os arquivos de histórico) e =processamento-historico-deputados.py= (processa os arquivos para estimar os salários).

Na experiência do autor, a API de histórico parece ter uma taxa de /timeouts/ elevada; assim, é possível que seja necessário executar o script várias vezes para obter todos os arquivos necessários.

O arquivo gerado com a estima de salários é salvo em =./input/deputados/historico-deputados-2025.json=.
Como nem todos os registros da listagem de deputados representam parlamentares em exercício, as estimativas feitas tentam evitar superestimar os custos de remunerações.
